...
class Mission():
    def __init__(self, player, target, mission_type, stay_time):
        ...
        if not stay_time:
            raise ValueError("need to %s n ticks, not %s" % (mission_type, str(stay_time)))
    ...
    def data_invariant(self):
        ...
        elif(self.stage != RETURN and self.stage != COMPETED and self.stage != ENROUTE and self.stage != ATTACK and self.stage != DEFENCE):
            raise ValueError("Stage is not %s, but: %s", (str([ENROUTE, RETURN, COMPETED, ATTACK, DEFENCE]), str(self.stage)))

        # mission type
        if(self.mission_type != ATTACK and self.mission_type != DEFENCE):
            raise ValueError("Mission type is not %s or %s but: %s", (ATTACK, DEFENCE, str(self.mission_type)))

        # travel tick
        if not type(self.travel_tick) == type(1):
            raise ValueError("Current tick is not a str: %s" % str(self.travel_tick))
        elif(self.travel_tick < 0):
            raise ValueError("Current tick is less than 0: %s" % str(self.travel_tick))
        elif(self.travel_tick > self.travel_time):
            raise MissionException("Current tick is larger than scheduled travel time: %s" % str(self.travel_tick))

        # player, target
        if not type(self.player) == type(Player(name="x")):
            raise ValueError("Player is not a Player: %s" % str(self.player))
        elif not type(self.target) == type(Player(name="x")):
            raise ValueError("Target is not a Player: %s" % str(self.player))
